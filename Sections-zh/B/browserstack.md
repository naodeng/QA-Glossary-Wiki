...
[BrowserStack](../B/browserstack.md) 是一个基于云的 **[cross-browser testing](../C/cross-browser-testing.md) 平台**，使开发人员和 QA 专业人员能够在各种浏览器、操作系统和真实移动设备上测试他们的网站和移动应用程序。它提供对大量浏览器和设备的访问，无需维护内部测试基础设施。
  借助[BrowserStack](../B/browserstack.md)，您可以使用[Selenium](../S/selenium.md)、Appium 和[Cypress](../C/cypress.md) 等流行框架进行**交互式[manual testing](../M/manual-testing.md)** 或运行**自动化测试**。它支持各种编程语言，包括 Java、Python 和 Ruby，允许无缝集成到现有的[test suites](../T/test-suite.md)。
...
[BrowserStack](../B/browserstack.md) 通过提供**对各种真实设备和浏览器的访问**来增强[software testing](../S/software-testing.md) 质量。这种多样性确保应用程序在密切反映最终用户环境的条件下进行测试，从而检测到在模拟器或有限设备上进行测试时可能会错过的**边缘情况问题**。
  ...
  **本地测试**允许对开发和登台环境进行安全测试，确保应用程序在发布到生产环境之前经过彻底审查。此功能对于识别特定于环境的 [bugs](../B/bug.md) 至关重要。
  [BrowserStack](../B/browserstack.md) **与流行的 CI/CD 工具**（如 Jenkins）集成，以及与 [Selenium](../S/selenium.md) 等框架的兼容性，可以无缝包含在自动化管道中。此集成支持**持续测试实践**，这对于敏捷和 DevOps 工作流程至关重要。
...
- **[Cross-browser Testing](../C/cross-browser-testing.md)**：跨多个浏览器及其版本验证您的 Web 应用程序的功能和设计。
  ...
  - **[Mobile App Testing](../M/mobile-app-testing.md)**：在大量真实的 iOS 和 Android 设备上测试本机和混合移动应用程序。
...
- **[Cross-browser Testing](../C/cross-browser-testing.md)**：跨多个浏览器及其版本验证您的 Web 应用程序的功能和设计。
  ...
  - **自动化[Screenshot Testing](../S/screenshot-testing.md)**：大规模捕获和比较屏幕截图以验证应用程序的视觉方面。
  - **[Mobile App Testing](../M/mobile-app-testing.md)**：在大量真实的 iOS 和 Android 设备上测试本机和混合移动应用程序。
  - **[Integration Testing](../I/integration-testing.md)**：将 [BrowserStack](../B/browserstack.md) 与 CI/CD 管道结合起来，将运行测试作为开发过程的一部分。
...
当然，**[BrowserStack](../B/browserstack.md)** 可用于[automated testing](../A/automated-testing.md)。它提供了一个基于云的平台，使您能够在各种浏览器和真实的移动设备上运行自动化测试。首先，您需要配置[test automation](../T/test-automation.md) 框架以与[BrowserStack](../B/browserstack.md) 的远程服务器连接。
  以下是使用 **[Selenium](../S/selenium.md) [WebDriver](../W/webdriver.md)** 和 **JavaScript** 的基本示例：
...
[BrowserStack](../B/browserstack.md) 可以与各种测试工具集成，以增强自动化并简化工作流程。以下是如何将其与一些常用工具集成：
  **Appium**：将[BrowserStack](../B/browserstack.md) 的Appium 服务器用于[mobile app testing](../M/mobile-app-testing.md)。使用您的访问凭据将 Appium 客户端中的 `remote_url` 设置为 [BrowserStack](../B/browserstack.md) 的端点。
  ...
  ```**TestCafe**：使用 [BrowserStack](../B/browserstack.md) 插件集成 TestCafe。在`.testcaferc.json` 文件中配置您的[BrowserStack](../B/browserstack.md) 凭据和所需的功能。
  ...
  ```**JUnit**：对于 JUnit 集成，将您的测试配置为使用 `RemoteWebDriver` 和所需功能连接到 [BrowserStack](../B/browserstack.md) 的 [Selenium](../S/selenium.md) 网格。
...
- **Appium**：对于移动应用程序测试，您可以使用 Appium 和 [BrowserStack](../B/browserstack.md) 在各种真实设备上运行自动化测试。
...
- **[Cypress](../C/cypress.md)**：[BrowserStack](../B/browserstack.md) 支持[Cypress](../C/cypress.md) 测试，允许您跨多个浏览器和操作系统运行它们。
  - **TestCafe**：您可以在 [BrowserStack](../B/browserstack.md) 上运行 TestCafe 脚本以利用其 [cross-browser testing](../C/cross-browser-testing.md) 功能。
  ...
  - **Puppeteer**：[BrowserStack](../B/browserstack.md) 使用 Puppeteer 提供对无头浏览器测试的支持，这对于快速反馈非常有用。
  - **Playwright**：集成 Playwright 测试以在 [BrowserStack](../B/browserstack.md) 上运行，以测试跨所有浏览器的现代 Web 应用程序。
  - **GitHub Actions**：通过将[BrowserStack](../B/browserstack.md) 与 GitHub Actions 集成以实现持续测试，自动化您的工作流程。
  - **Bitbucket Pipelines**：在 [BrowserStack](../B/browserstack.md) 中运行测试，作为 Bitbucket Pipelines CI/CD 流程的一部分。
...
- **本地测试**：通过在 [BrowserStack](../B/browserstack.md) 和本地计算机之间建立安全隧道，在防火墙后面或本地主机上安全地测试开发和登台环境。
  ...
  - **自动化[Mobile App Testing](../M/mobile-app-testing.md)**：使用 Appium、Espresso 和 XCUITest 框架在本机和混合移动应用程序上运行自动化测试。
...
[BrowserStack](../B/browserstack.md) 中的 **屏幕截图** 功能是一个允许用户跨不同浏览器和操作系统捕获和保存网页图像的工具。这对于 [visual regression testing](../V/visual-regression-testing.md) 特别有用，您需要确保您的 Web 应用程序在多个浏览器环境中外观和功能正常。
  要使用屏幕截图功能，您需要指定要测试的网页的 URL，以及浏览器和操作系统组合的列表。 [BrowserStack](../B/browserstack.md) 然后生成该页面的屏幕截图，就像它显示在所选浏览器和设备上一样。这些屏幕截图可以手动查看，也可以使用第三方工具以编程方式进行比较，以检测视觉差异。
